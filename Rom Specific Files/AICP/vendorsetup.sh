...
add_lunch_combo aicp_i9103-userdebug

#echo "Apply patch to frameworks/base"
#echo -n "Apply patch 0001-framework-base-patch.patch"
#(cd frameworks/base; git am ../../device/samsung/i9103/patches/aicp/0001-framework-base-patch.patch) > /dev/null 2>&1
#if [ $? == 0 ]; then
#	echo "     [DONE]"
#else
...
(cd external/skia; git am ../../device/samsung/i9103/patches/aicp/0005-external-skia-patch.patch) > /dev/null 2>&1
if [ $? == 0 ]; then
	echo "     [DONE]"
else
...
(cd external/chromium_org; git am ../../device/samsung/i9103/patches/aicp/0006-Work-around-broken-GL_TEXTURE_BINDING_EXTERNAL_OES-q.patch) > /dev/null 2>&1
if [ $? == 0 ]; then
  echo "     [DONE]"
else
...
#(cd bionic; git am ../device/samsung/i9103/patches/aicp/0007-Add-tegra2-to-bionic.patch) > /dev/null 2>&1
#if [ $? == 0 ]; then
#    echo "     [DONE]"
#else
